...
pub fn all_tools_with_runtime(
    ...
    let mut tool_arcs: Vec<Arc<dyn Tool>> = vec![
        ...
        Arc::new(EnvGetTool::new(security.clone())),
    ...
    if root_config.ntfy.enabled && !root_config.ntfy.targets.is_empty() {
        tool_arcs.push(Arc::new(NtfyTool::new(
            security.clone(),
    ...
    if root_config.a2a.enabled && !root_config.a2a.targets.is_empty() {
        tool_arcs.push(Arc::new(A2aAgentTool::new(
            security.clone(),
    ...
    if root_config.uptime_kuma.enabled && !root_config.uptime_kuma.targets.is_empty() {
        tool_arcs.push(Arc::new(UptimeKumaTool::new(
            security.clone(),
    ...
    if root_config.rss_feed.enabled && !root_config.rss_feed.feeds.is_empty() {
        tool_arcs.push(Arc::new(RssFeedTool::new(root_config.rss_feed.clone())));
    }
    ...
    if root_config.ralphy.enabled {
        tool_arcs.push(Arc::new(RalphyTool::new(
            security.clone(),
    ...
    if root_config.lifx.enabled {
        tool_arcs.push(Arc::new(LifxTool::new(
            security.clone(),
